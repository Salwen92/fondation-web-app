...
export const POST = withValidation(
  ...
  async (req: NextRequest, body: ClearJobsRequest) => {
    try {
      ...
      logger.info(`[Clear Stuck Jobs] Cleared ${result.clearedJobsCount} stuck jobs from ${result.repositoriesProcessed} repositories`);
      ...
      logger.error("[Clear Stuck Jobs] Error", error instanceof Error ? error : new Error(String(error)));
      return NextResponse.json(
