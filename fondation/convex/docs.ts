  ...
  handler: async (ctx, args) => {
    ...
    for (const file of args.files) {
      ...
      const normalizedContent =
        file.kind === 'yaml' ? file.content : normalizeMarkdown(file.content);
...
export const cleanupOldGenerations = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const oldJobs = await ctx.db
      ...
      .withIndex('by_repository', (q) => q.eq('repositoryId', args.repositoryId))
      .filter((q) =>
        q.and(q.eq(q.field('status'), 'completed'), q.neq(q.field('_id'), args.keepLatestJobId)),
      )
      .collect();
...
export const exportAll = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.jobId !== undefined) {
      ...
      const query = ctx.db.query('docs').withIndex('by_job', (q) => q.eq('jobId', jobId));
