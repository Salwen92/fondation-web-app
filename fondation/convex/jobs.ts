...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (activeJob) {
      throw new Error(
        'A job is already running for this repository. Please wait for it to complete or cancel it first.',
      );
    }
...
export const cancelJob = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      !['pending', 'claimed', 'cloning', 'analyzing', 'gathering', 'running'].includes(job.status)
    ) {
...
export const requestCancel = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      !['pending', 'claimed', 'running', 'cloning', 'analyzing', 'gathering'].includes(job.status)
    ) {
