...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const activeJob = await ctx.db
            ...
            .filter((q) => q.or(q.eq(q.field("status"), "pending"), q.eq(q.field("status"), "claimed"), q.eq(q.field("status"), "cloning"), q.eq(q.field("status"), "analyzing"), q.eq(q.field("status"), "gathering"), q.eq(q.field("status"), "running")))
            .first();
        if (activeJob) {
...
export const updateStatus = mutation({
    args: {
        jobId: v.id("jobs"),
        status: v.union(v.literal("pending"), v.literal("claimed"), v.literal("cloning"), v.literal("analyzing"), v.literal("gathering"), v.literal("running"), v.literal("completed"), v.literal("failed"), v.literal("canceled"), v.literal("dead")),
        callbackToken: v.string(),
        progress: v.optional(v.string()),
        currentStep: v.optional(v.number()),
        totalSteps: v.optional(v.number()),
        result: v.optional(v.union(v.object({
            success: v.boolean(),
...
export const clearStuckJobs = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const stuckJobs = await ctx.db
            .query("jobs")
            .withIndex("by_repository", (q) => q.eq("repositoryId", args.repositoryId))
            .filter((q) => q.and(q.eq(q.field("status"), "pending"), q.lt(q.field("createdAt"), fiveMinutesAgo)))
            .collect();
