...
export const heartbeat = mutation({
    args: {
        jobId: v.id("jobs"),
        workerId: v.string(),
        status: v.optional(v.union(v.literal("cloning"), v.literal("analyzing"), v.literal("gathering"), v.literal("running"))),
...
export const retryOrFail = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const baseDelay = Math.min(BACKOFF_BASE * Math.pow(2, attempts - 1), BACKOFF_MAX);
