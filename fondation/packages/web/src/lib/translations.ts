...
export function translateProgress(progress: string | undefined): string {
  ...
  if (lowerProgress.includes('initializing')) {
    return 'Initialisation...';
  }
  if (lowerProgress.includes('cloning')) {
    return 'Clonage du dépôt...';
  }
  if (lowerProgress.includes('analyzing') || lowerProgress.includes('analysis')) {
    return 'Analyse en cours...';
  }
  if (lowerProgress.includes('gathering')) {
    return 'Collecte des informations...';
  }
  if (lowerProgress.includes('generating')) {
    return 'Génération du cours...';
  }
  if (lowerProgress.includes('processing')) {
    return 'Traitement...';
  }
  if (lowerProgress.includes('running')) {
