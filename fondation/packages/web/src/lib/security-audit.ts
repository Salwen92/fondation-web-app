...
export class SecurityAuditLogger {
   ...
   */
  logTokenAccess(userId: string, action: string, resource?: string): void {
    this.logEvent(SecurityEventType.TOKEN_ACCESSED, action, {
      userId,
      resource,
   ...
   */
  logRateLimit(userId: string, remaining: number, limit: number): void {
    const severity = remaining === 0 ? SecurityEventSeverity.ERROR : SecurityEventSeverity.WARNING;
  ...
  private updateMetrics(event: SecurityEvent): void {
    ...
    if (
      event.type === SecurityEventType.API_RATE_LIMIT_WARNING ||
  ...
  private checkForSuspiciousActivity(event: SecurityEvent): void {
    ...
    const recentTokenAccesses = this.events.filter(
      (e) =>
