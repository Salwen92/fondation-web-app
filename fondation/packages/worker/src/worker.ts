...
export class PermanentWorker {
  ...
  get isHealthy(): boolean {
    ...
    return (
      this.isRunning &&
      memoryPercent < 1.0 &&
      (this.stats.total === 0 || timeSinceLastJob < 1800000 || uptime < 300000)
    ); // 30 minutes or 5 min startup grace
  }
  ...
  private validateExecutionEnvironment(): void {
    ...
    const isInsideDocker =
      process.env.DOCKER_CONTAINER === 'true' || require('node:fs').existsSync('/.dockerenv');
  ...
  async start(): Promise<void> {
    ...
    while (this.isRunning) {
      ...
      const success = await this.logger.safeExecute('poll-and-process', async () => {
  ...
  private async pollAndProcess(): Promise<boolean> {
    ...
    const claimedJob = await this.logger.safeExecute('claim-job', async () => {
  ...
  private async preValidateJob(job: Job): Promise<{
    ...
    if (!repository) {
      this.logger.logError(
        ...
        new Error(`Repository ${job.repositoryId} not found`),
        { jobId: job.id },
      );
    ...
    if (!user?.githubId) {
      this.logger.logError(
        ...
        new Error(`User not found or missing GitHub ID`),
        { jobId: job.id },
      );
      await this.failJob(
        job.id,
        `User not found or missing GitHub ID for repository ${repository.fullName}`,
      );
      return null;
    }
    ...
    const userGithubToken = await this.convex.query(api.users.getGitHubToken, {
      githubId: user.githubId,
    });
    this.logger.logInfo(
      `[PreValidate] GitHub token result: ${userGithubToken ? 'Found' : 'Not found'}`,
    );

    if (!userGithubToken) {
      this.logger.logError(
        'GitHub token not found',
        new Error(`No token for user ${user.githubId}`),
        { jobId: job.id },
      );
      await this.failJob(
        job.id,
    ...
    this.logger.logInfo(
      `[PreValidate] Starting decryption of token: ${userGithubToken.substring(0, 20)}...`,
    );
    this.logger.logInfo(
      `[PreValidate] ENCRYPTION_KEY available: ${process.env.ENCRYPTION_KEY ? 'YES' : 'NO'}`,
    );
    let token: string;
    try {
      ...
      this.logger.logInfo(
        `[PreValidate] ✅ Token decrypted successfully! Length: ${token.length}, starts: ${token.substring(0, 4)}...`,
      );
    } catch (error) {
      this.logger.logError('Token decryption failed', error as Error, { jobId: job.id });
      this.logger.logInfo(
        `[PreValidate] ❌ Failed to decrypt token: ${error instanceof Error ? error.message : String(error)}`,
      );
      await this.failJob(
        job.id,
        'Invalid GitHub token format. Please reconnect your GitHub account.',
      );
      return null;
    }
  ...
  private async processJob(job: Job): Promise<void> {
    ...
    try {
      ...
      const validationResult = await jobLogger.safeExecute('pre-validate-job', async () => {
      ...
      if (!validationResult) {
        this.logger.logError(
          'pre-validate-job',
          new Error(`Pre-validation failed for job ${job.id}`),
          { jobId: job.id },
        );
        return;
      }
      ...
      try {
        ...
        if (!userGithubToken) {
          ...
          if (!envToken) {
            this.logger.logError(
              'github-token-fetch',
              new Error('No GitHub token available anywhere'),
              { jobId: job.id },
            );
            throw new Error(
        ...
        this.logger.logInfo(
          `[ProcessJob] ✅ GitHub token prepared (length: ${userGithubToken.length})`,
        );
        // Phase 5: Clone repository
        this.logger.logInfo(`[ProcessJob] Phase 5: Starting repository cloning`);
        await jobLogger.safeExecute('update-job-status-cloning', async () => {
        ...
        this.logger.logInfo(
          `[ProcessJob] Cloning repo: ${repoUrl} (branch: ${repository.defaultBranch || 'main'})`,
        );
        ...
        await jobLogger.safeExecute('update-job-status-analyzing', async () => {
        ...
        this.logger.logInfo(
          `[ProcessJob] CLI execution params: repoPath=${repoPath}, prompt length=${job.prompt.length}`,
        );
        ...
        this.logger.logInfo(
          `[ProcessJob] ✅ CLI execution completed. Result success: ${result?.success}, documents: ${result?.documents?.length || 0}`,
        );
      ...
      this.logger.logError(
        `[ProcessJob] ❌ Job failed with error: ${error instanceof Error ? error.message : String(error)}`,
        error as Error,
        { jobId: job.id },
      );
      ...
      this.stats.total++;
      this.logger.logInfo(
        `[ProcessJob] ========== Job processing completed for ${job.id} ==========`,
      );
    }
  }
  ...
  private calculatePollingDelay(jobWasFound: boolean, wasError: boolean): number {
    ...
    let backoffDelay =
      this.config.pollInterval * this.backoffMultiplier ** this.consecutiveEmptyPolls;
