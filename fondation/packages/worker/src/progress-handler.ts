...
export class ProgressHandler {
  ...
  private static readonly STEPS_FR = [
   ...
   */
  static processProgress(input: string): ProgressInfo | null {
    // Clean input
    const cleaned = ProgressHandler.cleanInput(input);

    // 1. Try to parse FONDATION_STEP format (most reliable)
    const fondationStep = ProgressHandler.parseFondationStep(cleaned);
    if (fondationStep) return fondationStep;

    // 2. Try to parse "Étape X/Y" format (already formatted)
    const frenchStep = ProgressHandler.parseFrenchStep(cleaned);
    if (frenchStep) return frenchStep;

    // 3. Try to parse JSON log format
    const jsonStep = ProgressHandler.parseJsonLog(input); // Use original for JSON
    if (jsonStep) return jsonStep;

    // 4. Check if it's a status message
    const statusMessage = ProgressHandler.parseStatusMessage(cleaned);
   ...
   */
  static formatForUI(info: ProgressInfo): string {
    if (info.step > 0) {
   ...
   */
  static getStepNumber(info: ProgressInfo): number {
    return info.step;
  }
   ...
   */
  private static parseFondationStep(text: string): ProgressInfo | null {
    const match = text.match(/\[FONDATION_STEP_(\d+)\/(\d+)\]\s*(.*)/i);
    if (!match) return null;

    const step = Number.parseInt(match[1]);
    const totalSteps = Number.parseInt(match[2]);
    let description = ProgressHandler.cleanInput(match[3] || '');

    // Translate if in English
    description =
      ProgressHandler.TRANSLATIONS[description] ||
      description ||
      ProgressHandler.STEPS_FR[step - 1] ||
      description;
   ...
   */
  private static parseFrenchStep(text: string): ProgressInfo | null {
    const match = text.match(/Étape\s+(\d+)\/(\d+):\s*(.*)/i);
    if (!match) return null;

    return {
      step: Number.parseInt(match[1]),
      totalSteps: Number.parseInt(match[2]),
      message: match[3] || '',
      status: 'running',
    };
  }
   ...
   */
  private static parseJsonLog(text: string): ProgressInfo | null {
    if (!text.includes('{') || !text.includes('"msg"')) return null;
    ...
    try {
      ...
      if (msg.includes('FONDATION_STEP')) {
        return ProgressHandler.parseFondationStep(msg);
      }
   ...
   */
  private static parseStatusMessage(text: string): ProgressInfo | null {
    ...
    for (const [status, message] of Object.entries(ProgressHandler.STATUS_MESSAGES)) {
   ...
   */
  static processLineBuffer(buffer: string, onProgress: (info: ProgressInfo) => void): string {
    const lines = buffer.split('\n');
    const remainder = lines.pop() || '';

    for (const line of lines) {
      if (line.trim()) {
        const info = ProgressHandler.processProgress(line);
        if (info) {
          onProgress(info);
        }
      }
