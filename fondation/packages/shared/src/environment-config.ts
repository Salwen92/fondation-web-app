...
export class EnvironmentConfig {
  ...
  getDockerEnvironmentCheck(): DockerEnvironmentCheck {
    if (this._dockerCheck === null) {
      const hasDockerEnv = process.env.DOCKER_CONTAINER === 'true' || process.env.CONTAINER === 'true';
  ...
  getWorkerId(): string {
    return process.env.WORKER_ID || `worker-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  }
  ...
  validateProductionEnvironment(): EnvironmentValidationResult {
    if (this._productionValidation === null) {
      ...
      if (!this.isProduction()) {
        this._productionValidation = { valid: true, errors: [], warnings: ["Not running in production mode"] };
  ...
  validateDevelopmentEnvironment(): EnvironmentValidationResult {
    if (this._developmentValidation === null) {
      ...
      if (!this.isDevelopment()) {
        this._developmentValidation = { valid: true, errors: [], warnings: ["Not running in development mode"] };
